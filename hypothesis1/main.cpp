...
auto solve(int start) {
    ...
    Solution S;
    assert(fabs(optimal - sumFI) <= 1e-3);
    S.value = optimal;
    auto tmpS = curS;
    for (auto i : tmpS) {
        remove(i);
...
Solution res[thread_count];
double ans[N];
int ansV[N];
void rmain2(int b) {
    Solution cur, opt;
    for (int i = b; i < n; i += thread_count) {
        cur = F[b].rmain1(i);
        ans[i] = cur.value;
        ansV[i] = cur.S1.size() + cur.S2.size();
        opt = max(opt, cur);
...
int main(int argc, char **argv) {
    ...
    if (method.type == "MBG") {
        ...
        for (int i = 0; i < thread_count; i++) {
            thr[i] = std::thread(rmain2, i);
        }
        for (int i = 0; i < thread_count; i++) thr[i].join();
        vector <double> values;
        Solution opt;
        for (int i = 0; i < thread_count; i++) opt = max(opt, res[i]);
        int maxN = 0;
        for (int i = 0; i < n; i++) maxN = max(maxN, ansV[i]);
        for (auto i : opt.S1) values.push_back(ans[i]);
        for (auto i : opt.S2) values.push_back(ans[i]);
        sort(values.begin(), values.end());
        fout << std::fixed << std::setprecision(3) << values[(values.size() + 1) / 2] / values.back() << endl;
        fout << std::fixed << std::setprecision(3) << 1. * maxN / (opt.S1.size() + opt.S2.size()) << endl;
    }
}